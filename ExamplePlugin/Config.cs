...
namespace HedgehogUtils
{
    public static class Config
    {
        ...
        {
            return HedgehogUtilsPlugin.instance.Config.Bind<bool>("Super Form", "Invincible", true, $"Whether the Super form grants complete invincibility. Without invincibility, the Super form grants {Forms.SuperForm.StaticValues.superSonicArmor} armor. Host's config takes priority. Default is true.");
        }
        public static ConfigEntry<float> SuperFormDuration()
        {
            return HedgehogUtilsPlugin.instance.Config.Bind<float>("Super Form", "Duration", Forms.SuperForm.StaticValues.superSonicDuration, "How long the Super form will last in seconds. Host's config takes priority. Default is 50.");
        }
